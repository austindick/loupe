...
2. Fix property values and CSS variable swaps in-place — do NOT perform structural rebuilds; if fixing an item requires adding or removing a FauxDS component, mark as ESCALATE
...
- `sourceFilePath` — the built source file from Agent 4
...
- Section files confirmed at `.loupe/sections/[sectionScope]/`

## Execution

### Step 1: Read spec files

Read `layoutPath` (LAYOUT.md) and `designPath` (DESIGN.md). Check if INTERACTIONS.md exists at `interactionsPath`; if yes, load it.

### Step 2: Read the built source file

Read `sourceFilePath` completely. Locate the section matching `sectionScope` using the same boundary-finding strategy as Agent 4:
...
- **LAYOUT.md Token Quick-Reference:** per row — verify the element uses the specified `var(--faux-*)` token
- **LAYOUT.md Annotated CSS:** per CSS property per rule — verify the value uses `var(--faux-*)` with correct token name; verify `/* Figma: Xpx */` comment is present
- **DESIGN.md Component Mapping:** per row — verify the FauxDS component, variant, and each prop matches exactly
- **DESIGN.md Typography:** per row — verify the preset, size, weight, and line-height each match
- **DESIGN.md Color Tokens:** per row — verify the element uses `var(--faux-color-*)` with the correct token
- **DESIGN.md Unmapped Values:** per entry — verify the TODO comment exists in locked format `{/* TODO: unmapped — closest FauxDS: ... */}`; mark PASS if present, FAIL if missing; do NOT fix the underlying value
- **INTERACTIONS.md (if loaded):** per state entry — verify the state is implemented
...
- If fixing requires adding or removing a FauxDS component: mark as ESCALATE, skip the fix
...
Verify that `.loupe/sections/[sectionScope]/LAYOUT.md` and `.loupe/sections/[sectionScope]/DESIGN.md` exist — they should already be present from Agents 1 and 2. If INTERACTIONS.md was loaded, verify it exists too. Do not move files — they are already at the correct path.
...
No file artifact beyond the fixed `sourceFilePath`. Summary report is conversation output only. Section files confirmed at `.loupe/sections/[sectionScope]/`. The orchestrator reads the summary from conversation to determine pipeline success.
